...
fn good_specs_processing_1() {
    ...
    let specs_result = SpecsStorage::from_payload_prelude_cut(&payload[3..], &Encryption::Sr25519);
...
fn good_meta_processing() {
    let payload_hex =
        std::fs::read_to_string("for_tests/load_metadata_westendV9430_Alice-sr25519.txt").unwrap();
